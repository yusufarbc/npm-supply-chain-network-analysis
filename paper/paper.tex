...
After normalization, the values were weighted to calculate the Composite Risk Score (BRS):
\[
\text{BRS} = 0.5 \cdot in' + 0.2 \cdot out' + 0.3 \cdot btw'
\]
In the formula, in-degree ($w_{in}=0.5$) has the highest weight. This is related to the number of projects the package will affect being a systemic risk indicator. Betweenness ($w_{btw}=0.3$) was included in the model as the second factor. The weight coefficients were determined empirically based on the assumption that "impact radius" is a more critical risk factor than "attack surface" in supply chain attacks.
